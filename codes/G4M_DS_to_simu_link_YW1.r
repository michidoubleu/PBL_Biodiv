...
g4mid_to_simuid_YW1 <- function(DS.res = out.res,
         lab,
         project,
         curr.SCEN1,
         curr.SCEN2,
 ...
 # DS.res=readRDS(paste0('./input/ds_test/output_isimip_9863.000557.RData'))[[3]]$out.res
  ...
  G4M.res <-
    ...
    # mutate(forest_old_ha=forest_old_ha*0.001) %>% ## YW: convert G4M result to 1000ha, to be in line with DS result
    # mutate(forest_new_ha=forest_new_ha*0.001) %>% ## As communicated with Andrey on 10 July 2024
  ...
  DS.res.g4mland <-
    DS.res %>% mutate(
      lu.from = recode(
        ...
        "PriFor" = "G4Mland",
        "MngFor" = "G4Mland",
        "RstLnd" = "G4Mland",  #YW: Now actually there is no "RstLnd" in the sent DS.res (goes to YesAffor->OthNatLnd or RstLnd_NoAffor)
        "protected_priforest" = "G4Mland"  #YW: 20231014 add this also into G4Mland
      ),
      lu.to = recode(
        lu.to,
        "OthNatLnd" = "G4Mland",
  ...
  check1.init.g4mland <- DS.init.g4mland.forest
  check1.excess.forest <- excess.forest
    ...
    DS.init.g4mland <-
      curr.init.DS %>%  mutate(
        lu.from = recode(
          ...
          "forest_old_ha" = "G4Mland",
          "protected_priforest" = "G4Mland",#YW: 20231014
          "OthNatLnd" = "G4Mland")) %>%
      group_by(REGION, ns, lu.from, times) %>%
      summarise(value = sum(value))%>%
      subset(lu.from == 'G4Mland')
  ...
  result_G4M_to_SimU <- list()
  result_G4M_to_SimU[[1]] <- DS.res.G4M
  result_G4M_to_SimU[[2]] <- check1.init.g4mland
  result_G4M_to_SimU[[3]] <- check1.excess.forest

  return(result_G4M_to_SimU)
}
