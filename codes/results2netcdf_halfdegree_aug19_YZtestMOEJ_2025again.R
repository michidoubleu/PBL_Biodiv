...
result2netcdf_path <- "e:\\IIASA\\Model\\downscale_biodiversity_G4M\\"
setwd(result2netcdf_path)
...
Yazhen_test_path <- "e:\\IIASA\\Model\\downscale_biodiversity_G4M\\Yazhen_test\\"
...
# YW: define MOEJ scenarios------#
# See backup for more alternative scenario selections

# project <- "ScenTest10FP"
# lab <- "04112023"
project <- "ScenDriver5new"
lab <- "13012025"
scenarios = data.frame(Scenario=c("SSP2_SCEN_BASE","SSP2_SCEN_CLIM","SSP2_SCEN_CONS","SSP2_SCEN_CLIM_CONS","SSP2_SCEN_CLIM_CONS_SDGL"),
                       SCEN1=c("SSP2","SSP2","SSP2","SSP2","SSP2"),
                       SCEN2=c("SCEN","SCEN","SCEN","SCEN","SCEN"),
                       SCEN3=c("BASE","CLIM","CONS","CLIM_CONS","CLIM_CONS_SDGL"))

# END YW define scenarios -------#
...
for (sss in 1:nrow(scenarios)) {
  ...
  # ftarget = paste(Yazhen_test_path,"/","/csv_G4M_DS_link_output/","/",project,"_",lab,"_",scenarios$Scenario[sss],"_LULUC.csv",sep="")
  # ftarget = paste(Yazhen_test_path,"/","/csv_G4M_DS_link_output/","/",project,"_",lab,"_",scenarios$Scenario[sss],"_LULUC_2024Mar06.csv",sep="") ## YW 20240306: updated CSVs with the REGION37 duplicated value addressed
  ftarget = paste(Yazhen_test_path,"/","/csv_G4M_DS_link_output/","/",project,"_",lab,"_",scenarios$Scenario[sss],"_LULUC_2025Sept01.csv",sep="") ## YW 20240306: updated CSVs with the REGION37 duplicated value addressed
  
  res_file <-read.csv(ftarget ,stringsAsFactors=FALSE)
  
  ## ---YW 20240305 for diagnosis---
  res_file_LU = aggregate(res_file[,c(2,7:16)] ,
  ...
  # for (david_version in david_versions[2:5]) {
    ...
    # time_date_tag = "23Jan2024"
    time_date_tag = "06Mar2024"
    # fsave = paste0(target_path,"BendingTheCurveFT-LCproj-GLOBIOM-",RCP,"-",save_date_tag,"-",time_date_tag,".nc")
    # fsave = paste0(Yazhen_test_path,"/netcdf_output/","BendingTheCurveFT-LCproj-GLOBIOM-",RCP,"-",save_date_tag,"-",time_date_tag,".nc") # Yazhen: change output netCDF path to my test folder
    # fsave = paste0(Yazhen_test_path,"/netcdf_output_Mar2024update/","BendingTheCurveFT-LCproj-GLOBIOM-",scenarios$Scenario[sss],"_",david_version,"_",time_date_tag,".nc") # Yazhen: change output netCDF path to my test folder
    fsave = paste0(Yazhen_test_path,"/netcdf_output_Sept2025update/","BendingTheCurveFT-LCproj-GLOBIOM-",scenarios$Scenario[sss],"_",david_version,"_",time_date_tag,".nc") # Yazhen: change output netCDF path to my test folder
